...
app.use((req, res, next) => {
    ...
    // Allow static assets
    const staticFileExtensions = ['.css', '.webp', '.js', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.woff', '.woff2', '.ttf', '.otf', '.eot', '.ico', '.pdf'];
...
function sendServerStartEmail() {
    ...
    fs.readFile(path.join(__dirname, 'email.html'), 'utf8', (err, data) => {
        ...
        let htmlContent = data
            ...
            .replace('<p>It was downloaded by IP address <strong>${realIp1}</strong></p>', `<p>Server IP address :<strong>${getServerIP()}</strong></p>`)
            ...
            .replace('href="https://ipinfo.io/${realIp2}"', 'href="https://snyders.xyz"')
